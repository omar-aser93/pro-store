...
// Generate a PayPal access token test
test('generates a PayPal access token', async () => {
  const tokenResponse = await generateAccessToken();
  console.log(tokenResponse);
  //Types test: Should be a string that is not empty
...
test("creates a PayPal order", async () => {
  ...
  //Types test:  Ensure the order response contains expected fields
...
// test of Capture payment with a mock order 
