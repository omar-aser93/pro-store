...
const AdminNav = ({ className, ...props}: React.HTMLAttributes<HTMLElement>) => {
  ...
  const links = [
    ...
    { title: 'Users', href: '/admin/users' }, { title: 'Newsletter', href: '/admin/newsletter' }
  ]; 
  const pathname = usePathname();             //usePathname() hook to get the current url pathname

  return (
    <nav className={cn('flex items-center rtl:gap-6 space-x-4 lg:space-x-6', className)} {...props} >
